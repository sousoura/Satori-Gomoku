...
from dlgo.agent.smart_utsuho import Smart_Utsuho
from dlgo.agent.smart_sanyousei import Smart_Sanyousei
...
def main():
    ...
    # game = game.apply_move(Move(Point(row=5, col=5)))
    # game = game.apply_move(Move(Point(row=3, col=1)))
    # game = game.apply_move(Move(Point(row=7, col=1)))
    """
        测试用
    """
    ...
    bot_1 = Smart_Utsuho(1)
    bot_1 = Smart_Sanyousei(2)
    ...
    while not game.is_over():
        ...
        if game.next_player == gotypes.Player.black:
            if mode == "h v b" or mode == "h v h":
                ...
                # 让机器人下这一步
                move, estimation = bot_1.select_move(game)
                print("black_bot:", estimation)
                exhibitor.display(game, gotypes.Player.black)
        else:
            if mode == "b v b" or mode == "h v b":
                # 给bot棋盘 bot生成一个动作
                move, estimation = bot_2.select_move(game)
                print("white_bot:", estimation)
        ...
        # 测试
        # heuristic_estimation(game)

    print(game.winner(), "win this game")
    while exhibitor.gate:
        exhibitor.display(game, False)
